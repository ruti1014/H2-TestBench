...
void setup() {
  ...
  MicroSD.vendorID("SDHC");//max 8 chars
  MicroSD.productID("USB_MSC");//max 16 chars
  MicroSD.productRevision("1.0");//max 4 chars
  MicroSD.onStartStop(onStartStop);
  MicroSD.onRead(onRead);
  MicroSD.onWrite(onWrite);
  MicroSD.mediaPresent(true);
  MicroSD.begin(DISK_SECTOR_COUNT, DISK_SECTOR_SIZE);
  USBSerial.begin();
  USB.begin();
}
